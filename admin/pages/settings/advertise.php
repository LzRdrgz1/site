    ...
    $html->element("form", array("method" => "post", "action" => $html->Url("actions")), array(
        ...
        $html->element("div", array("class" => "panel-body"), array(
            $html->element("div", array("class" => "row"), array(
                ...
                $html->element("div", array("class" => "col-lg-4 col-md-6"), array(
                    $html->element("div", array("class" => "form-group"), array(
                        $html->label("vast_ad_opt", "control-label", "VAST AD"),
                        $html->tip("Whether to show the VAST ad code in streaimg page"),
                        $html->element("select", array("class" => "form-control", "id" => "vast_ad_opt", "name" => "vast_ad"), array(
                            $html->option("Enable", "enable", Data::Get("vast_ad")),
                            $html->option("Disable", "disable", Data::Get("vast_ad")),
                        )),
                    )),
                    $html->element("div", array("class" => "form-group"), array(
                        $html->label("vast_ad_code", "control-label", "VAST AD Code"),
                        $html->tip("VAST ad code to show  in streaimg page"),
                        $html->element("textarea", array("class" => "form-control", "id" => "vast_ad_code", "name" => "vast_ad_code", "rows" => "6"), array(
                            base64_decode(Data::Get("vast_ad_code")),
                        )),
                    )),
                )),
            )),
...
$html->script('
    ...
    $("#vast_ad_opt").bind("change",function(){
        $value = $(this).val();
        if ($value == "enable") {
            $("#vast_ad_code").prop("disabled", false);
        } else {
            $("#vast_ad_code").prop("disabled", true);
        }
    });
    $("#pop_ad_opt").trigger("change");
    $("#banner_ad_opt").trigger("change");
    $("#vast_ad_opt").trigger("change");
');
